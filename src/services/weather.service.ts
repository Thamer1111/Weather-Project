...
export const getWeather = async (userId: mongoose.Types.ObjectId, rawLat: number, rawLon: number): Promise<FormattedWeatherData> => {
  const { lat, lon } = roundCoordinates(rawLat, rawLon);
  const now = new Date();
  ...
  let currentSourceForProcessing: FormattedWeatherData['source'] = 'openweather'; 
  let formattedData: FormattedWeatherData;

  if (weatherDoc) {
    currentSourceForProcessing = 'cache'; 
    ...
    try {
      ...
      currentSourceForProcessing = 'openweather'; 
      formattedData = {
        source: 'openweather',
